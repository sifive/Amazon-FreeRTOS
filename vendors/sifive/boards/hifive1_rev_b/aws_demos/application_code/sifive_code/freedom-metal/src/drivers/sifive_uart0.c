...
#define UART_REG(offset)   ((control_base + offset))
...
{
    control_base = __metal_driver_sifive_uart0_control_base(uart);
...
{
    struct __metal_driver_sifive_uart0 *uart = (void *)guart;
    control_base = __metal_driver_sifive_uart0_control_base(guart);
...
static void pre_rate_change_callback(void *priv)
{
    ...
    control_base = __metal_driver_sifive_uart0_control_base((struct metal_uart *)priv);
...
void __metal_driver_sifive_uart0_init(struct metal_uart *guart, int baud_rate)
{
    ...
    // set interrupt watermark to one
    UART_REGW(METAL_SIFIVE_UART0_RXCTRL) = (UART_RXEN | UART_TXCNT(0));
